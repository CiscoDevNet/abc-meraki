...
def diff_networks_full_vlan(orgs):
    for org in orgs:
        if "ECMS" in org["name"]:
            networks = dashboard.organizations.getOrganizationNetworks(org["id"])
            ...
            for network in networks:
                try:
                    ...
                    with open(f'meraki_settings/actual/vlans/{network["name"]}vlans.json', 'w') as outfile:
                        json.dump(vlans, outfile, indent=4)
                    
                    copy_gold_image_for_match('vlans', f'{network["name"]}vlans.json')
                except Exception as e:
                    print(e)

def copy_gold_image_for_match(setting_fldr, file):
    src_dir = f'meraki_settings/gold/{setting_fldr}'
    src_file = f'gold_{setting_fldr}.json'
    # create a dir where we want to copy and rename
    src_file = os.path.join(src_dir, src_file)
    dest_file = os.path.join(src_dir, f'{file}')
    shutil.copy(src_file,dest_file) #copy the file to destination dir
